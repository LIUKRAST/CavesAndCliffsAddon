...
public class IcycaveStructure extends CavesAndCliffsUpdateAddonModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public IcycaveStructure(CavesAndCliffsUpdateAddonModElements instance) {
		super(instance, 204);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 300000) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("caves_and_cliffs_update_addon", "icycave"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("icycave"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("caves_and_cliffs_update_addon:icycave"),
					configuredFeature);
		}
	}
	@SubscribeEvent
